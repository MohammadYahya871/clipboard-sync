...
public sealed class LanServer : IAsyncDisposable
{
    ...
    public async Task SendClipboardEventAsync(ClipboardEvent clipboardEvent, byte[]? imageBytes, CancellationToken cancellationToken = default)
    {
        ...
        _logStore.Info(
            $"Starting image transfer {transferId} ({imageBytes.Length} bytes, {totalChunks} chunks)");
        ...
        for (var index = 0; index < totalChunks; index++)
        {
            ...
            // Yield so the Android reader can drain chunks without socket pressure.
            await Task.Yield();
        }
        ...
        _logStore.Info($"Finished image transfer {transferId}");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (socket.State == WebSocketState.Open && !cancellationToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                {
                    // transfer_chunk is extremely chatty and was flooding the UI log list.
                    if (!string.Equals(envelope.Type, "transfer_chunk", StringComparison.Ordinal))
                    {
            ...
            {
                _logStore.Warn($"Ignored malformed protocol envelope: {exception.Message}");
            }
        }
    }
}
